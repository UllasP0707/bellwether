...
## How one behaviour becomes one message

![Bellwether architecture](docs/img/architecture.png)

Solid blue is the hot path — a vendor event on its way to a decision. Dashed
teal is state. Dotted purple is the batch path recomputing the same events.
`traceparent` rides the Kafka headers, so a single trace id spans all four
services.
...
strategies over that one catalog and the same pure `score_events()` — the
dotted lines out of **the invariant** in the middle. That constraint is the
point of the project, and it is measured rather than asserted: 1,986 events, 0
disagreements, largest delta 0.0.

## Deployed

![Bellwether on AWS](docs/img/aws.png)

Applied to a real AWS account, verified, and destroyed — 102 resources. Each
pod is drawn touching only the stores its IAM role actually permits, which is
the claim rather than the decoration: the scorer cannot read the raw archive,
and the archive is the only store that holds addresses rather than tokens.
Confirmed with `iam simulate-principal-policy` on 11 of 11 allow-and-deny
decisions. See [infra/README.md](infra/README.md), including the bug this found
that two weeks of `terraform validate` could not.

Both diagrams are generated by [`scripts/architecture.py`](scripts/architecture.py)
rather than drawn by hand, so they cannot quietly go stale:
